// Контентный скрипт: только визуализация огонька и плашки с ответом
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "START_CAPTURE") {
    setIndicatorColor('#D4AC0D'); // Желтый — скриншот сделан и ушел в Gemini
    if (sendResponse) sendResponse();
    return true;
  }

  if (message.type === "SHOW_ANSWER") {
    setIndicatorColor('#229954'); // Зеленый — ответ от Gemini получен
...
function showMicroBadge(msg) {
  ...
  setTimeout(() => {
    ...
    setIndicatorColor('#555555'); // Возврат в серый режим ожидания
  }, 3500);
}